...
;ADDED FOR RACE COMPATIBILITY
	...
	int raceIndex = 0
	while(raceIndex < RaceList.GetSize())
		if(Target.GetActorBase().GetRace() == (RaceList.GetAt(raceIndex) AS Race))
			CureRace = (RaceList.GetAt(raceIndex) AS Race)
			Target.SetRace(VampireList.GetAt(raceIndex) AS Race)
		endif
		raceIndex += 1
	endwhile
...
;ADDED FOR RACE COMPATIBILITY
	...
	int raceIndex = 0
	while(raceIndex < RaceList.GetSize())
		if(Player.GetRace() == (VampireList.GetAt(raceIndex) AS Race))
			Player.SetRace(RaceList.GetAt(raceIndex) AS Race)
		endif
		raceIndex += 1
	endwhile
